...
class CalendarDayLabelView: UIView {
    ...
    var weekDayView: CalendarCellWeekView?
    
    required init?(coder aDecoder: NSCoder) {
        super.init(coder: aDecoder)
    }

    init(_ date: Date, _ isCalendar: Bool = true) {
    ...
    func layoutView(isCalendar: Bool = true) {
        ...
        for i in 0..<2 {
            var cellView: UIView
            if i == 0 {
                let weekView = CalendarCellWeekView(currentDate, true, isCalendar)
                cellView = weekView
                weekLabelView = weekView
            }
            else {
                let dayView = CalendarCellWeekView(currentDate, false, isCalendar)
                cellView = dayView
                weekDayView = dayView
            }
            addSubview(cellView)
